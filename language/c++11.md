...
<p style="color:red">You're discouraged to use std::mutex directly. Please use pr::mutex instead. 
The following is to illustrate how pr::plain_mutex wrapped in pr::mutex is implemented. </p>

<p style="color:red">Generally speaking, prefer using a lock rather than the mutex directly, because when there's exception the lock::~lock() will be called to unlock the mutex.</p>
...
<p style="color:red">You're discouraged to use std::condition directly. Please use pr::condition instead. 
The following is to illustrate how pr::condition is implemented. </p>
